...
<div class="container mx-auto p-4">
    ...
    <form action="{{ route('produtos.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="number" step="0.01" name="preco" class="border rounded w-full p-2" value="{{ old('preco') }}">
            @error('preco') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        <div>
            <label class="block">Descrição</label>
            <textarea name="descricao" class="border rounded w-full p-2">{{ old('descricao') }}</textarea>
